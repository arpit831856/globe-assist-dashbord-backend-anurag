...
<div class="page-content comp-page">
    ...
    <div class="content-header mb-4 d-flex justify-content-between align-items-center flex-wrap gap-2">
        <div>
            <h2 class="fw-bold mb-1">Booking-List</h2>
        </div>
    ...
    <div>
        <p class="comp-toolbar-title">Booking Records</p>
            ...
            <td>
                ...
                <form action="{{ route('admin.booking.approve', $booking->id) }}" method="POST">
                    ...
                    <button type="submit" class="btn btn-success btn-sm d-inline-flex align-items-center gap-1">
                        <svg class="approve-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" aria-hidden="true">
...
<div id="assignModal" style="display:none; position:fixed; top:0; left:0; width:100%; height:100%; background:rgba(0,0,0,.5); z-index:9999;">
    
    <div style="background:#fff; width:500px; padding:20px; margin:80px auto; border-radius:10px; position:relative;">

        <h4>Assign Partner</h4>

        <button onclick="closeModal()" 
            style="position:absolute; top:10px; right:15px; border:none; background:none; font-size:22px;">
            ×
        </button>

        <p id="bookingInfo"></p>

        <form method="POST" action="{{ route('admin.booking.saveAssign') }}">
            @csrf
            ...
            <div class="mb-3">
                ...
                <select name="partner_id" id="partnerList" class="form-control" required>
...
document.querySelectorAll('.openAssignModal').forEach(button => {
    ...
    button.addEventListener('click', function () {
        ...
        fetch("{{ url('admin/get-partners') }}/" + bookingId)
        .then(response => {
            if (!response.ok) {
        ...
        .then(data => {
            ...
            if(data.length === 0){
                options = `<option value="">No Partner Available</option>`;
            }
            ...
            document.getElementById('partnerList').innerHTML = options;
        })
        .catch(error => {
            console.log(error);

            document.getElementById('partnerList').innerHTML =
                `<option value="">Error Loading Partners</option>`;
        });
...
function showToast(message, type = 'success') {
    ...
    toast.className = "page-alert";

    if(type === "success"){
        toast.classList.add("success-alert");
        toast.innerHTML = `
            <span>✔ ${message}</span>
        `;
    } else {
        toast.classList.add("error-alert");
        toast.innerHTML = `
            <span>✖ ${message}</span>
        `;
    }
