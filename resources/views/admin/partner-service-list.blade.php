...
<div id="toastContainer"></div>
  ...
  <div id="Partner-users-page" class="page-content">
    <div class="page-header">
      <h4 class="page-title">Partner Services</h4>
      <!-- Trigger Button -->
    ...
    <div class="card mb-3">
      <div class="card-body">
        <div class="row g-3 align-items-center">
          <!-- Search Bar -->
          <div class="col-md-8">
            <form action="{{ route('partner.service.list') }}" method="GET">
          ...
          <table class="table table-hover" id="PartnerUsersTable">
            ...
            <tbody>
              @foreach($partners as $index => $partner)
                <tr>
                  @php
                    $servicePhoto = is_array($partner->photos) ? ($partner->photos[0] ?? null) : null;
                  @endphp
                  {{-- @dd($partner); --}}
                  <td>{{ $index + 1 }}</td>
                  ...
                  <td>
                    <img
                      src="{{ $servicePhoto ? asset('storage/' . $servicePhoto) : asset('assets/globe assist logo.png') }}"
                  ...
                  <td>{{ $partner->price }}</td>
<td>
@if($partner->service_date)
    {{ \Carbon\Carbon::parse($partner->service_date)->format('d-m-Y') }}
@else
    -
@endif
</td>                <td>{{ ucfirst($partner->location) }}</td>
                  <td>
                    @if(strtolower($partner->status) === 'active')
                      <span class="badge bg-success">Active</span>
                    @elseif(strtolower($partner->status) === 'pending')
                  ...
                  <td class="text-center">
                    ...
                    @else
                      <form action="{{ route('partner.service.approve', $partner->id) }}" method="POST">
                        @csrf
